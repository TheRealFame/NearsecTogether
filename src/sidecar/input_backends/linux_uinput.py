...
W3C_MAPPING = {
    ...
    4: uinput.BTN_TL,
    5: uinput.BTN_TR,
    # 6 and 7 are Triggers (Handled below via ABS_Z and ABS_RZ)
    8: uinput.BTN_SELECT,
    9: uinput.BTN_START,
    10: uinput.BTN_THUMBL,
    11: uinput.BTN_THUMBR,
    # 12-15 are D-Pad (Handled below via ABS_HAT)
    16: uinput.BTN_MODE,
}
                ...
                if current_mode == "kbm":
                    ...
                    if hasattr(uinput, str(key_code)) and getattr(uinput, str(key_code)) in KBM_EVENTS:
                        if event_type == "keydown":
                            gp.emit(getattr(uinput, key_code), 1)
                        if event_type == "keyup":
                            gp.emit(getattr(uinput, key_code), 0)
